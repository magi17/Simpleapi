...
app.get('/gemini', async (req, res) => { // Updated endpoint
  ...
  if (!ask) {
    return res.status(400).json({ error: 'The ask  parameter is required. you can use ?ask= or ?ask=&imageurl=?' });
  }
...
// Start the server
app.listen(port, () => {
  console.log(`Server is running on port ${port}`); // Updated port in log
});
